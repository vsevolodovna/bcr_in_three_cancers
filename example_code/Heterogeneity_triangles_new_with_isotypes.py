...
for f in files:
    ...
    df=df[['sequence','clonefraction','clone_id', 'filename','cancer', 'patient', 'organ', 'sample','isotype']]
    data=data.append(df)
...
org1_threshold=2
org2_threshold=1
isotype_dominance_threshold=0.6

dominant_isotype_list=['A','M','G','no']
color_dict={"A":"royalblue","E":"brown","M":"forestgreen","G":"tomato","D":"yellow","no":"grey"}
...
data=data[data['isotype'].isin(dominant_isotype_list)]
...
for pat in patients:
    ...
    if (org2 in list(df['organ'].unique())) and (df[df['organ']==org1]['sample'].nunique()>=3):
        ...
        for comb in org1_samples_combinations:
            ...
            #FOR COLORING BY DOMINANT ISOTYPE CALCULATIONS
            df['isotype_share']=df.groupby(['clone_id','isotype'])['sequence'].transform('count')/df.groupby(['clone_id'])['sequence'].transform('count')
            df['dominant_isotype']=df.assign(dominant_isotype=df['isotype'].where(df['isotype_share']>=isotype_dominance_threshold)).fillna('').groupby('clone_id')['dominant_isotype'].transform(lambda x: x.max() if x.any() else 'no') #СЛОЖНА.. но вроде правильно
            df['dominant_isotype_color']=[dominant_isotype_color(x) for x in df['dominant_isotype']]
        
            #Create pivot table
            df=df.groupby(['clone_id','cluster_size','organ','sample','org2_share','dominant_isotype','dominant_isotype_color']).agg({'sequence':'count'})
            df=df.reset_index()
            df=df.pivot(index=['clone_id','cluster_size','org2_share','dominant_isotype','dominant_isotype_color'],columns='sample',values=['sequence'])
            df.columns = df.columns.droplevel(0)
            df = df.reset_index()
            df=df[['clone_id','cluster_size','org2_share','dominant_isotype','dominant_isotype_color']+org1_samples]
            ...
            colors=list(df['dominant_isotype_color']) #colors=list(df['org2_share']) was transferred lower in this file
            labels=list(df['dominant_isotype'])
            ...
            #THIS LINE WOULD BE ENOUGH IF WE DID NOT NEED LEGEND
            #tax.scatter(points,s=sizes,vmax=max(colors), colormap=plt.cm.Reds, c=colors,alpha=0.5,label=labels) #for tum vs tum add colorbar=True for legend originally
            
            
            #ADD THIS PART TO FACILITATE LEGEND
            ...
            for iz in list(df['dominant_isotype'].unique()):
                    ...
                    colors=list(df2['dominant_isotype_color'])
                    ...
                    tax.scatter([mass_center],marker="*",s=300,color=color_dict[iz],edgecolor='black')
                
            plt.xlim(-0.15,1.15)
            plt.ylim(-0.1,0.95)
            tax.set_title(pat+", "+org2+" vs "+org1+" samples\n"+stat, fontsize=15, pad=30)
            ...
            tax.savefig("Pictures/Heterogeneity_triangle_isotype_coloring_"+pat+"_"+org2+"_vs_"+org1+"_samples_all_data_normalized_"+str(idx)+".png",bbox_inches='tight')
            tax.savefig("/home/sofyakrasik/Pictures/BCR_Manuscript_Pictures_20220331/Heterogeneity_triangle_isotype_coloring_"+pat+"_"+org2+"_vs_"+org1+"_samples_all_data_normalized_"+str(idx)+".pdf",bbox_inches='tight')
